...
var TailToStoreHandler = class {
  constructor(store, onset, worker) {
    ...
    if (!spanId)
      return;
    this.#rootSpanId = spanId, this.#traceId = traceId, this.#startMs = toMs(onset.timestamp);
  ...
  attributes(event) {
    ...
    Object.keys(attrs).length > 0 && this.#track(this.store.mergeAttributes(traceId, spanId, attrs));
  }
  return(event) {
    let { traceId, spanId } = ids(event), pending = spanId ? this.#spans.get(spanId) : void 0;
    spanId && pending && event.event.info?.type === "fetch" && this.#track(
...
${event.event.stack}` : head;
    pending && (pending.errored = !0, pending.outcome = "error", pending.error = head), this.#track(
      this.store.appendLog({
        traceId,
        spanId: spanId ?? null,
        tsMs: toMs(event.timestamp),
  ...
  async outcome(event) {
    let endMs = toMs(event.timestamp), traceId = this.#traceId ?? event.spanContext.traceId, root = this.#rootSpanId ? this.#spans.get(this.#rootSpanId) : void 0;
    ...
    this.#rootSpanId && this.#invocationBody !== null && this.#track(
      this.store.appendLog({
        traceId,
  ...
  #close(traceId, spanId, pending, endMs, attributes) {
    pending.closed || (pending.closed = !0, this.#track(
